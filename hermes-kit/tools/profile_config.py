...
# WHAT DOES NOT TRAVEL. Four top-level keys, each measured, and nothing else.
NOT_PROJECTED = {
    ...
    "gateway",
    # `plugins.enabled` only means something next to `HERMES_HOME/plugins`, and
    # a role's home has none: the promises guard is mounted over
    # `/opt/data/plugins`, which is the DEFAULT profile's home
    # (`hermes:hermes_cli/plugins.py:1369`). Projecting the list would enable a
    # plugin the profile cannot see -- installed, off, and the config saying it
    # is on, which is the exact failure that guard exists to catch.
    "plugins",
}
...
#
# FOUR KEYS DO NOT TRAVEL, each for a reason that is in tools/profile_config.py:
# api_server and platforms (the listener and the channels are the default
# profile's -- and the pin below is what keeps this profile served), gateway
# (multiplex_profiles is read once, at boot) and plugins (its list only means
# something next to a plugins/ dir this home does not have).
#
