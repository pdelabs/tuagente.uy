...
- `reopen_path` → la absoluta, la que usás vos si después tenés que **abrir
  el archivo de nuevo**.
...
**va con `--attachment`**, no a `workspace/interno/`:

```bash
python3 /opt/kit/skills/deliverable/deliver.py \
  --title "Post 1 — Instagram" --kind borrador \
  --attachment /opt/data/workspace/interno/post-1.png <<'MD'
...
Todo trabajo que nace de un flujo lleva su `--flow` — sin excepción.
