...
client_summary: "Pasa la foto o el PDF de tus facturas a la planilla —fecha, proveedor, número y monto— y te muestra lo que no pudo leer antes de que cuente."
name: invoices-to-data
...
# invoices-to-data — la factura entra a la planilla sin que nadie tipee
...
```bash
python3 /opt/kit/skills/invoices-to-data/scripts/verify_rows.py < rows.json
```
...
- **`problems`** — contradicciones, no opiniones: el dígito verificador del RUT
...
- **`review`** — preguntas: el IVA no da ni 22% ni 10% (puede ser una factura
  con las dos tasas, puede ser un dígito mal leído), la diferencia entra en el
  redondeo. No son errores; se miran y se sigue.
- **`unread`** — las columnas que vinieron vacías. Es tu propia declaración,
...
- Si el mes se está armando ahora, la tanda es un entregable: se guarda con la
  skill `deliverable`, y **con `--flow planilla-del-mes` si nació de ese flujo**.
...
mirar. Ahí va aprobación, con la skill `approval` y en el ticket que ya estás
trabajando.
