---
title: Crear flujos
client_summary: "Cuando le pedís un trabajo repetitivo nuevo, lo deja armado como un flujo que ves en tu pestaña Flujos."
name: flow
...
# flow — convertir un pedido repetitivo en un trabajo con nombre
...
(sin carpeta, sin FLOW.md, sin pestaña) y el cron sale con `deliver=origin`,
...
2. **Con qué.** Si el trabajo necesita una conexión que no está, decilo y
   ...
   script programa el gatillo igual y te devuelve `missing_connections` para que
...
devuelve `missing_connections` con la frase exacta para decirle a tu cliente que
...
saber que entendiste. Si el gatillo es `request`, no corresponde.

## Uso

```bash
python3 /opt/kit/skills/flow/create_flow.py \
  --slug resumen-semanal --name "Resumen semanal de ventas" \
  --client-summary "Todos los lunes a las 8 te espera el resumen de la semana." \
  --trigger schedule --detail "Todos los lunes a las 8:00" \
  --cron "0 8 * * 1" \
  --connections google-workspace \
  --skills deliverable <<'MD'
# Cómo trabajo este flujo
1. ...pasos concretos...
2. Entrego con `deliverable --flow resumen-semanal`.
...
- `--connections` es OBLIGATORIO. Si el trabajo no toca ninguna, poné
  `--connections ninguna`. Si toca una que no está conectada, declarala igual:
  ...
  **El script chequea cuáles de esas conexiones están puestas de verdad** y, si
  falta alguna, te devuelve `missing_connections` y un `tell_the_client`. Eso no
...
con `deliverable --flow <slug>` para que caigan en la carpeta del flujo.
...
pedí lo que falte con la skill `capability`. Nunca te quedes callado: el cliente
...
`flows/PREFERENCIAS.md` son las reglas del cliente que valen para todos sus
...
El portal muestra el cuerpo del FLOW.md en la página del flujo ("Cómo lo
...
Cerrá cada FLOW.md con una sección `## Notas técnicas`: ahí van las skills
exactas, los flags (`deliverable --flow <slug>`) y cualquier precisión de
