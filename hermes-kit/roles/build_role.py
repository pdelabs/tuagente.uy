...
# THE ONE THING A SECONDARY PROFILE'S CONFIG HAS TO SAY, and it is not
# decoration: without it the gateway serves NONE of that role's adapters.
#
# `API_SERVER_KEY` lives in the container's environment (secrets.env is the
# env_file of both services), and the engine's loader puts env vars ABOVE
# config.yaml: a profile whose config does not say otherwise comes out with
# api_server ENABLED (hermes:gateway/config.py:2131-2155). With
# `gateway.multiplex_profiles` on, the default profile owns the single HTTP
# listener and serves every role through /p/<role>/ -- so a secondary profile
# that also declares a port-binding platform is a config error, and the
# gateway says so at every boot and moves on without it:
#
#   WARNING gateway.run: Skipping secondary profile 'accounting' due to
#   port-binding config error: ...
#
# The explicit `enabled: false` is the documented way out: the engine pops an
# `_enabled_explicit` marker off the block and lets the pin beat the env var
# (the comment at gateway/config.py:2144-2152 describes this exact file). It
# costs the role nothing -- /p/<role>/ is answered by the default profile's
# listener either way, which is why the skip looked benign for weeks: the URL
# kept working while the profile's own adapters never started.
#
# AND IT IS THE ONLY THING A DISTRIBUTION CAN SAY. The rest of a role's config
# -- the model, the toolsets, the gate hooks, the disabled engine skills -- is
# the AGENT's, and a distribution belongs to no agent. That copy happens at the
# hire, where data/config.yaml is at hand: `tools/profile_config.py` projects it
# and `tools/hire-role.sh` rewrites this file with the result before installing.
# The pin's own text lives there too, so the two writers cannot drift apart.


def write_profile_config(role: str, dest: Path) -> None:
    """The profile's config.yaml, which exists for one line (see above)."""
...
# config.yaml is the exception the engine makes: it is distribution-owned but
# `hermes profile update` PRESERVES it unless --force-config, on the assumption
# that a profile config carries user overrides. Ours carries the pin that keeps
# the profile servable, so hire-role.sh --update passes --force-config.
distribution_owned:
  - SOUL.md
  - config.yaml
  - skills/
...
def build(role: str, out_root: Path) -> Path:
    ...
    write_profile_config(role, dest)
