...
It runs on every answer before it is persisted (`session.BEFORE_PERSIST`), so
what the portal reads back is the corrected text and not what the model said.
...
everything, and NEVER created a flow. In the Flujos tab it
...
code: either there's an active FLOW.md with a cron on it, or there isn't. So
instead of asking the model to remember, we look at the disk and, if what it
said isn't there, the text goes out with a correction attached that the
client can't miss.
...
WHAT CHANGED WHEN FLOWS MOVED INTO THE ENGINE. It used to read a `data/` dir:
`flows/<slug>/FLOW.md` for the folder and `cron/jobs.json` for whether the
trigger was actually alive, because on Hermes those were two things that could
disagree — folder yes, cron no, and the card said "active". Here the file IS
the schedule (`core/flows.py`), so there is one thing to read and half the
check's old job no longer exists.
"""
import re
import unicodedata
...
def live_flows():
    """The flows that will actually run: active, and on the clock.

    A `request` flow doesn't count — it starts when the client asks, so an
    answer claiming something repeats is not backed by one.
    """
    return [{"slug": f.slug, "name": f.name, "cron": f.cron}
            for f in flows.read_all()
            if f.status == "active" and f.trigger == "schedule"]
...
def review(text):
    ...
    Fails on the side of silence: text that doesn't match any of the shapes
    returns None. This check can't be the thing that breaks a response.
    """
...
# Registered at import, like `core/compaction.py` and `core/turn_usage.py`:
# `server/app.py` imports this module for the side effect, before the first
# turn. The guard used to be a kit plugin's, because flows were the kit's; the
# flows are the engine's now and so is the check that reads them.
session.BEFORE_PERSIST.append(check)
