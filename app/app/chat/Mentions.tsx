...
// Composer: `@` names a file, `#` names a ticket. Two prefixes for the two
// kinds of thing the client can point at, and the hint under the box says so
// in those words.
//
// Picking one inserts its id/path and the chat draws it as a chip.
...
export function mentionAt(text: string, caret: number):
  ...
  // Only at the start of a word: an address typed mid-sentence
  // ("escribile a ana@…") must not open the picker.
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-xl border border-black/10 bg-white">
      <p className="border-b border-black/[0.06] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-ink-soft">
        {kind === "ticket" ? "Tareas del tablero" : "Archivos del agente"}
      </p>
      {items === null ? (
