...
client_summary: "Antes de una acción sensible, arma el pedido que ves en tu pestaña de Aprobaciones: qué quiere hacer y qué pasa si decís que sí."
name: approval
...
# approval — pedir permiso de una manera que se entienda
...
```bash
python3 /opt/kit/skills/approval/format_request.py \
  --what "Enviar el mail de respuesta a Acme SA" \
  --if-approved "Se envía el mail tal cual está abajo, desde la casilla comercial" \
  --if-rejected "No se envía nada; espero tu corrección" \
  --why "Es el primer contacto con esa empresa" <<'MD'
Hola Juan,
