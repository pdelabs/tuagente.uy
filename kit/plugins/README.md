...
| `instagram` | no | the other half of that account: the comments AND the direct messages read every fifteen minutes, answered, hidden or replied to behind the gate, a lead onto the board — `core`, a skill and the flow that runs it |
...
craft and a `flows/` surface — `instagram`, every fifteen minutes —
