...
# kept.
#
# "Calls to the model" is TWO routes, not one, and each is wired below with its
# own reason: the chat (data/config.yaml) and image generation
# (OPENROUTER_BASE_URL in data/.env — the image plugin resolves its endpoint on
# its own and ignores the model block). Miss either and the proxy is up, the
# operator reads "on", and that traffic is neither traced nor priced in
# costs.jsonl.
#
...
case "$ACTION" in
  status)
    ...
    echo -n "  image route      : "
    ssh "$HOST" "grep -h '^OPENROUTER_BASE_URL=' $DIR/data/.env 2>/dev/null | tr '\n' ' ' || true"
    ssh "$HOST" "grep -qh '^OPENROUTER_BASE_URL=' $DIR/data/.env 2>/dev/null" \
...
REMOTE
    ...
    # "Unknown provider").
    # ONE HOME. This used to sweep `data/profiles/*/config.yaml` as well,
    # because a hired role's home was its own and the engine merged NOTHING
    # from the parent -- a config only the default profile got left every
    # teammate talking straight to OpenRouter while the operator read "the
    # proxy is in the middle". There are no profiles; there is one config.
...
PY"
    ...
    #
    # AND IT HAS TO BE A `.env` FILE, NOT THE CONTAINER ENVIRONMENT. The engine
    # reads credentials through a secret scope built from `<home>/.env`, and a
    # scope miss returns the default instead of falling through to os.environ --
    # fail-closed on purpose (hermes:agent/secret_scope.py:123-190). secrets.env
    # reaches the process and the turn never sees it. Measured 24/8 on the local
    # agent, back when `gateway.multiplex_profiles` was still on; it is off now
    # and nobody has re-measured it, which is a reason to keep writing the file,
    # not to stop.
    echo "→ pointing image generation at the proxy"
    ssh "$HOST" "python3 - <<'PY'
import pathlib
env = pathlib.Path('$DIR/data/.env')
...
env.write_text('\n'.join(lines) + '\n')
if fresh:
    # It holds a URL, not a credential, and the engine runs as another user:
    # readable, or the proxy is simply not there.
    env.chmod(0o644)
PY"
  ...
  off)
    # Both halves come back out: the chat's base_url and the image plugin's
    # OPENROUTER_BASE_URL. Leaving the second one behind would point image
    # generation at a container that is about to be stopped, and the failure
    # reads as "the image tool broke" with no proxy in sight.
