...
|---|---|---|---|---|
| tuagente.uy's own agent (Luis' Mac) | `engine/instances/tuagente/`, container `tuagente-tuagente`, `:8652`/`:8653` | its own `agent/SOUL.md` on `core:base v2` | **engine** (Pydantic AI, `engine/`) | 14/9: built as the first workload of the engine (`docs/own-agent-plan.md`). Identity «Tu Agente» for «tuagente.uy», plugins `approval,memory,image,social`, brand in `workspace/marca/brand.md`. The daily flow `posteo-instagram-diario` was created through the chat, ran its first round in the chat turn and once more from the Flows tab's run-now, which found today's post and did not duplicate it. `portal-check` **0 failures**. Nothing is published; Luis publishes from Posteos |
| east-v2 (Luis' Mac) — **the kit's validation agent, not a client** | `east-v2`, local docker, `:8662`/`:8663`, `~/Desktop/Luis/Projects/east-v2` | **v13** — installed by `new-agent.sh` + `install.sh` | `v2026.7.30` | 30/8: built from zero to validate `interview-production` + `drive-inbox` before anything goes remote (`docs/east-requirements.md` §5). Baptized «Selastian» through the portal API, works for «East Comunicación». `policy/capabilities/purchased.json` = `["drive-inbox", "interview-production"]` → **9 plugins** (six system + `transcribe` via the base row + the two bought), 11 kit skills, 10 flows. `agent-check` **30 ok · 0 warn · 0 failures**, `portal-check` **16 ok · 0 warn · 0 failures**. Two requirements run end to end on real material: a YouTube interview → transcript + ten timecoded zócalos, and a Radio Viva URL → headline+copy draft, both left pending approval on their own ticket. The Drive trigger was armed with fake folder ids, fired once, left the loud ticket, and was returned to `request` — **there is no cron on this agent now**. Own OpenRouter key `east-v2-dev`, limit US$10, **US$0.167 spent** over four runs. It holds a real key and a real `TUAGENTE_MODELS_KEY`: if it stops being useful, revoke the key, do not just delete the folder |
...
> English-layout migration (`tools/migrate-agent-to-english.sh`) and a SOUL v13
...
> see.** It renamed only the host half of the compose bind (leaving
> `- ./policy:/opt/politica`, which mounts fine and silently empties the
> client's permissions and capability cards); its ssh round trip rewrote
> `data/` from `10000:10000` to `501:0`, which takes the agent's ability to
> write its own state away; and `ssh` inside the cleanup loop ate the rest of
> its own input, so 1 of 8 retired paths was deleted and `politica/` survived.
> All three are fixed and re-validated end to end. The lesson is the test
> fixture's: it was written from the post-translation world (`/opt/policy`
> inside the container), which is a shape no agent needing this migration has,
> so the assertion that no `politica` survived passed over a file that could
> not exist. **A migration's fixture has to be copied off a real pre-migration
> agent, not written from memory.**
