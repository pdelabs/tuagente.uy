...
(`kit/adapter/portal_adapter.py:2535-2551`, `docs/PENDING.md:857-863`,
...
`kit/compose/config.base.yaml:1-4` ships `provider: openrouter` with no
...
by the capability: `kit/capabilities/catalog.json:239` carries
...
`TELEGRAM_ALLOWED_USERS` (`kit/new-agent.sh:119-140`) — and not in the
...
`kit/capabilities/catalog.json:243`, in the `=== images ===` block of
...
note, `kit/capabilities/catalog.json:243`:
