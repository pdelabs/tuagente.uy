...
lives in `kit/` inside the tuagente.uy repo (`worktree-pivot-equipo`
...
kit/tools/preview-agentito.mjs` → localhost:8077 (every variant of
...
kit/plugins/ with 7 plugins (5 system + transcribe +
...
The registry is 13 plugins; `kit/skills/` is down to `no-images` and
...
landed in **notes/plugin-system-plan.md**: registry at kit/plugins/,
...
6. **CLOSED 24/8, and the answer was no.** The sweep found exactly two
   ...
   **`agente-east` IS a client agent and was NOT deleted.** It is not the
   ...
   agent — `kit/` is the archived pre-monorepo kit repo and
