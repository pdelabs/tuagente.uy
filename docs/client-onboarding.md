...
- **Server: one VPS per client** — Hetzner CX23 (2 vCPU, 4 GB, 40 GB,
  ...
  - Onboarding is run by `kit/deploy-remote.sh`.
...
4. **The bot's photo**: when the client names their agent, the portal
   ...
   uploaded with `kit/tools/avatar-bot.py` (Telethon via MTProto — the
...
`kit/new-agent.sh`, with the slug, the name and the ports already resolved.
...
`kit/deploy-remote.sh` does the whole thing: it uploads a staging copy,
...
- **`agent-check.py` needs the WHOLE tree, not `data/`.** The script's own
  ...
  python3 kit/tools/agent-check.py <local-mirror>/data
  ```
...
Nothing gets copied by hand: `kit/install.sh <path>/data` puts it
...
keeps reading the old one. Run `kit/tools/migrate-agent-to-english.sh
...
reinstall that goes with it) is in `kit/fleet.md`, under "Migrating a
...
1. **Put our client secret on the agent**, which is our half:
   ...
   `kit/connections/google-workspace.md`.
...
```bash
# local agent
kit/tools/reset-agent.sh --local <agent-path> --delivery
# agent on the VPS
kit/tools/reset-agent.sh <host-ssh> [slug] --delivery
```
...
```bash
python3 kit/tools/portal-check.py --key <API_SERVER_KEY> --delivery \
