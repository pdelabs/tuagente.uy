...
- **Runs are rows**: `flow_runs(slug, scheduled_at, session_id, started_at, finished_at, status running|paused|ok|error, error, manual)`, primary key `(slug, scheduled_at)`. Every transition writes an event (`flow.started`, `flow.paused`, `flow.finished`, `flow.failed`). A run that stops at the approval gate is `paused`, and becomes `ok` or `error` only when the client's answer lets it finish, so Activity shows them and a failed run is visible without any prose asking the agent to say so.
