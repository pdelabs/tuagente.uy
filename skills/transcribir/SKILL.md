---
titulo: Transcripción de audio y video
para_cliente: "Convierte una entrevista, reunión o nota de voz en texto."
name: transcribir
...
# transcribir — de audio o video a texto
...
Devuelve un JSON con `transcripcion` (la ruta del `.txt` con el texto completo),
un `vistazo` de los primeros caracteres, la duración y el costo. **El texto
completo está en el archivo — leelo de ahí**, el vistazo es solo para confirmar
que salió bien.

- `--idioma`: default `es`. Solo cambialo si el audio está en otro idioma.
- `--salida`: dónde dejar el `.txt`. Sin esto, queda al lado del archivo
  original con sufijo `.transcripcion.txt`.
